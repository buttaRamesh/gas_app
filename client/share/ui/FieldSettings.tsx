...
const mockOrderBookApi = {
  getFieldDefinitions: async () => ({
    data: {
      fields: [
        { field_name: 'order_no', label: 'Order Number', field_type: 'text', required: true },
        { field_name: 'consumer_name', label: 'Consumer Name', field_type: 'text', required: true },
        { field_name: 'consumer_number', label: 'Consumer Number', field_type: 'text', required: true },
        { field_name: 'mobile_number', label: 'Mobile Number', field_type: 'phone', required: false },
        { field_name: 'book_date', label: 'Booking Date', field_type: 'date', required: true },
        { field_name: 'product', label: 'Product', field_type: 'text', required: false },
        { field_name: 'quantity', label: 'Quantity', field_type: 'number', required: false },
        { field_name: 'amount', label: 'Amount', field_type: 'number', required: false },
        { field_name: 'delivery_address', label: 'Delivery Address', field_type: 'text', required: false },
        { field_name: 'notes', label: 'Notes', field_type: 'text', required: false },
...
const mockFieldConfigurationApi = {
  getByUploadType: async (uploadType: string): Promise<{ data: FieldConfig[] }> => ({
    data: []
  }),
  create: async (data: any) => ({ data: { ...data, id: Date.now() } }),
  update: async (id: number, data: any) => ({ data: { ...data, id } }),
  deleteByUploadType: async (uploadType: string) => ({
    data: {
      success: true,
      message: 'Settings deleted',
      deleted: { field_configurations: 0, column_mappings: 0 }
    }
  }),
};
...
const FieldSettings = () => {
  ...
  const loadFieldConfigurations = async () => {
    setLoading(true);
    try {
      // 1. Fetch all available fields (without upload_type filter to get ALL fields)
      const fieldsResponse = await mockOrderBookApi.getFieldDefinitions();
      ...
      const configsResponse = await mockFieldConfigurationApi.getByUploadType(uploadType);
      const existingConfigs: FieldConfig[] = configsResponse.data;
      ...
      const configMap = new Map(
        existingConfigs.map((config) => [
          config.field_name,
          {
            is_included: config.is_included ?? true,
    ...
    try {
      ...
      for (const config of fieldConfigs) {
        ...
        if (config.configuration_id) {
          // Update existing
          updatePromises.push(
            mockFieldConfigurationApi.update(config.configuration_id, configData)
          );
        } else {
          // Create new (both for included and excluded fields)
          createPromises.push(mockFieldConfigurationApi.create(configData));
        }
      }
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      const response = await mockFieldConfigurationApi.deleteByUploadType(uploadType);

      showSnackbar(
        `All settings for ${UPLOAD_TYPE_OPTIONS.find((opt) => opt.value === uploadType)?.label} deleted successfully`,
        "success"
      );
      setDeleteDialogOpen(false);
