...
import { StatCard, FormDialog } from "../../components/common";
  ...
  const fetchAvailableRoutes = async () => {
    try {
      ...
      const allRoutes = Array.isArray(response.data?.results)
        ...
        : [];
      const unassigned = allRoutes.filter((route) => !route.delivery_person);
  ...
  return (
    <Box sx={{ minHeight: "100vh", bgcolor: "grey.100", py: 4 }}>
      <Container maxWidth="lg" sx={{ px: 2 }}>
        ...
        <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", md: "repeat(3, 1fr)" }, gap: 3, mb: 3 }}>
          <StatCard
            icon={<RouteIcon />}
            iconColor="info"
            value={person.assigned_routes_count || 0}
            label="Assigned Routes"
            description="Total routes assigned to this person"
          />
          <StatCard
            icon={<ConsumersIcon />}
            iconColor="success"
            value={person.total_consumers || 0}
            label="Total Consumers"
            description="Total consumers across all routes"
          />
          <StatCard
            icon={<PersonIcon />}
            iconColor="primary"
            value={assignedRoutes.reduce((sum, route) => sum + route.area_count, 0)}
            label="Total Areas"
            description="Total delivery areas covered"
          />
        </Box>
      ...
      <FormDialog
        ...
        title={`Assign Routes to ${person.name}`}
        onSubmit={handleAssignRoutes}
        loading={assigning}
        submitLabel={`Assign ${selectedRoutes.length} Route(s)`}
        disableSubmit={selectedRoutes.length === 0}
        maxWidth="md"
      >
