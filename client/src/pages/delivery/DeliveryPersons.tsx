...
import { useDeliveryPersons, useSearch } from "@/hooks";
...
export default function DeliveryPersons() {
  ...
  const deliveryPerms = useResourcePermissions('delivery_persons');

  const { query: searchQuery, setQuery: setSearchQuery, filteredItems: filteredPersons } = useSearch({
    items: deliveryPersons,
    searchKeys: (person) => [person.name],
  });
      ...
      <Container maxWidth="xl" sx={{ px: 2 }}>
        ...
        {filteredPersons.length === 0 ? (
          <Box sx={{ textAlign: "center", py: 16 }}>
            ...
            {!searchQuery && deliveryPerms.canCreate() && (
              <Button
                variant="contained"
                startIcon={<AddIcon />}
                  ...
                  <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", mb: 2.5 }}>
                    ...
                    <Box sx={{ display: "flex", gap: 0.5, ml: 1 }}>
                      ...
                      <IconButton
                        ...
                        onClick={(e) => {
                          e.stopPropagation();
                          navigate('/delivery-persons/consumers', { state: { personId: person.id } });
                        }}
                        sx={{
                          transition: "all 0.2s",
                          "&:hover": {
